...
| `cameras/dualcam_fast.py` | High-rate dual-camera GUI (Thorlabs CS165MU + IDS DCC1545M). The FPS control commands a **fixed hardware frame rate** (IDS `is_SetFrameRate`, Thorlabs `frame_rate_control_value`), clamps exposure to the frame period (`clamp_fps_exposure`), and re-applies it after every ROI change; the display refresh is decoupled at `DISPLAY_FPS_CAP` (30). IDS ring buffer depth is `IDS_RING_BUFFERS` (12). At record-stop it captures a **hardware frame clock** (frame counter + device timestamp) — the recorded `_timestamps.npy` jitter is mostly software dequeue-noise (OS scheduling), so analysis should prefer the hardware clock. Writes `<video>_timestamps.npy` (sw), `<video>_hwclock.npz` (hw_frame_count/hw_timestamp_ns/ring_in_use), and the `<video>.json` sidecar (schema v2, with `hw_measured_fps`, `dropped_frames`, `ring_peak_in_use`). |
...
| `scripts/dipole/dipole_sideband_model.py` | Step 2 of the naive-vs-measured ACTS workflow: turns the naive rotating-field E-field ellipse into the dipole **libration sideband** (ωφ=√(d·\|E₊\|/I), co-rotating projection — NOT ⟨\|E\|⟩) and the headline **d-inference error** `d_inferred/d_true = \|E₊\|_true/E_believed`. Analytic + numeric EOM→PSD. Called by `plot_naive_vs_measured_acts.py` or standalone on an assumed ellipse. See `scripts/dipole/README.md`. |
